...
controller.getAllAssociations = async (req, res) => {
  ...
  include: [
    ...
    console.log('Associations:', associations); // Pour vérifier les données récupérées
...
controller.getUserLists = async (req, res) => {
  try {
    ...
    const userWithLists = await users.findOne({
      ...
      include: [{
        model: gameList,
        as: 'game_lists', // Doit correspondre à l'alias dans l'association
        attributes: ['game_list_id', 'name', 'description'],
        through: { attributes: [] }, // Pour ne pas inclure les attributs de la table intermédiaire
      }],
      attributes: ['user_id', 'username', 'mail'],
    });
...
controller.getListUsers = async (req, res) => {
  try {
    ...
    const listWithUsers = await gameList.findOne({
      ...
      include: [{
        model: users,
        as: 'users', // Doit correspondre à l'alias dans l'association
        attributes: ['user_id', 'username', 'mail'],
        through: { attributes: [] },
      }],
      attributes: ['game_list_id', 'name', 'description'],
